...
Item *readItems(const char *path, size_t *itemNum)
{
	...
	*itemNum = shape[0];
	...
	for(size_t i=0; i<shape[0]; i++)
	{
		...
		//Get item type
		itemList[i].type = strNoSpaces(itemData[i*shape[1] + 1])[0];
					...
					Item *itemList, size_t itemNum)
{
	...
	while(mainLoop == 0)
	{
		...
		while(1)
		{
			clear();
			printf(" ---------- Character Creation ----------\n");
			printf(" -> What is your main weapon?\n");
			printf(" 1) Sword \n");
			printf(" 2) Bow \n");
			char option = getAChar();
			if (option == '1')
				player->mainHand = searchItemByName(itemList, itemNum, "Sword");
			else if (option == '2')
				player->mainHand = searchItemByName(itemList, itemNum, "Bow");
			else
				continue;
			break;
		}
