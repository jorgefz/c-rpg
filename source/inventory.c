...
Matches with values of eqp_slots enum.
*/
const char *eqp_slot_names[] = {
				"Main Hand",
				"Secondary Hand",
				"Head",
				"Chest",
...
{
	for(size_t i=0; i<EQP_SLOTS; i++){
		//Skip if empty
		if(!player->eqp_slots[i]){
			continue;
		}
		printf(" %"SZ_FMT") %s: %10s\n", i+1, eqp_slot_names[i], player->eqp_slots[i]->name);
	}
}
...
void inv_print(Charac *player)
{
	...
	for(size_t i=0; i<EQP_SLOTS; i++){
		...
		printf(" - %s: %s\n", eqp_slot_names[i], player->eqp_slots[i]->name);
	}
...
{
	if( player->eqp_slots[slotId] == NULL ){
		return NULL;
	}

	inv_add(player->inv, player->eqp_slots[slotId]);
	player->eqp_slots[slotId] = NULL;
...
Charac *inv_equip(Charac *player, size_t itemId)
{
	...
	size_t slotId = item->eqp_type;
	printf(" Equipping %s (%s) on %s\n", item->name, eqp_slot_names[item->eqp_type], eqp_slot_names[slotId]);
